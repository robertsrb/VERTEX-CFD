#ifndef VERTEXCFD_PARAMETERPACK_HPP
#define VERTEXCFD_PARAMETERPACK_HPP
...
//---------------------------------------------------------------------------//
// Get an element from a parameter pack. Note that this has been inserted in
// the main VertexCFD namespace as it will be used heavily.
template<std::size_t N, class ParameterPack_t>
KOKKOS_FORCEINLINE_FUNCTION
    typename std::enable_if<Utils::is_parameter_pack<ParameterPack_t>::value,
                            typename ParameterPack_t::template value_type<N>&>::type
    get(ParameterPack_t& pp)
{
    return static_cast<typename ParameterPack_t::template element_type<N>&>(pp)
        ._m;
}

template<std::size_t N, class ParameterPack_t>
KOKKOS_FORCEINLINE_FUNCTION typename std::enable_if<
    Utils::is_parameter_pack<ParameterPack_t>::value,
    typename ParameterPack_t::template const_value_type<N>&>::type
get(const ParameterPack_t& pp)
{
    return static_cast<const typename ParameterPack_t::template element_type<N>&>(
               pp)
        ._m;
}
...
template<typename ParameterPack_t, std::size_t N, typename T, typename... Types>
void fillParameterPackImpl(ParameterPack_t& pp,
                           ...
                           const Types&... ts)
{
    ...
    fillParameterPackImpl(
        pp, std::integral_constant<std::size_t, N - 1>(), ts...);
}
...
void fillParameterPack(ParameterPack_t& pp, const Types&... ts)
{
    fillParameterPackImpl(
        pp,
        std::integral_constant<std::size_t, ParameterPack_t::size - 1>(),
        ts...);
}
...
#endif // end VERTEXCFD_PARAMETERPACK_HPP
